...
context("Page Builder - Export/Import of Pages", () => {
    let id, pageTitle;

    beforeEach(() => {
        id = uniqid();
        pageTitle = `Test Page ${id}`;
    ...
    it("should be able to export and import a page", () => {
        ...
        // Search for page.
        cy.findByTestId("default-data-list.search", { timeout: 15000 }).within(() => {
            cy.findByPlaceholderText(/Search.../i).type(pageTitle);
        });

        cy.findAllByTestId(/ui-data-table-row-index-.*/)
            .first()
            .within(() => {
                cy.findByText(pageTitle).should("exist");
                cy.get(`input[type="checkbox"]`, { timeout: 15000 }).click({ force: true });
            });
        ...
        // Export ready
        cy.findByTestId("export-pages.export-ready-dialog", { timeout: 15000 }).should(
            "be.visible"
        );
        ...
        cy.findByTestId("import-pages.loading-dialog").within(() => {
            cy.findByText("All pages have been imported", { timeout: 15000 }).should("be.visible");
