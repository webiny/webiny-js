...
- Compose CSS class names with a helper, never string concatenation (`+`) or template literals. In packages that depend on `@webiny/admin-ui`, use its `cn` helper (`clsx` + `tailwind-merge`). In admin-ui-agnostic packages (e.g. `@webiny/lexical-editor`), import `clsx` directly (instead of pulling in `@webiny/admin-ui`) but alias it as `cn` — `import cn from "clsx"` — so the call site reads the same everywhere.
