...
1. **Domain → infra leak.** `api-core` implements `RequestInitializer` (an event-handler-core contract) to schedule the WCP license refresh — domain reaching *out* to infra.
2. **Infra → domain leak (already present).** `event-handler-aws` is *not* transport-agnostic: `ApiGatewayIdentityLoaderDecorator`, `ApiGatewayTenantLoaderDecorator`, and `S3TenantLoaderDecorator` all import `@webiny/api-core/features/requestContext`. The transport package knows about the domain's identity/tenant model.
