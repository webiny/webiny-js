...
Nothing implements that abstraction. It is *populated by a side effect* — a hook resolves the model
...
An earlier pass grepped by *name* (`*Initializer|*ContextualSchema`) and conflated distinct
...
initializer *"MUST run before the GraphQL initializer, whose resolvers resolve
`DeleteModelOperations`"*. But those resolvers call
...
**There *is* a real ordering constraint, but it is internal to crud.** `DisableModelFeature.register()`
...
container.registerInstance(Abstraction, new BlockActionIfModelDisabled(ops.isModelBeingDeleted.bind(ops)));
```

That is why it is invoked from *inside* `createDeleteModelCrud.init()`, immediately after
...
```ts
interface IGraphQLContextualSchema { build(ctx): Promise<GraphQLSchema>; }
```
...
- **Do not add a cache to a provider without checking the layer below.** `ModelsFetcher` already
  ...
  therefore stateless.
- **If a provider *does* memoize, register it per request, never at root.** A root-scoped singleton
...
- **Fix the name collision as you go.** Every package has two exports with the same name — the
  `ModelFactory` *definition* and the resolved-instance *abstraction* (`FileModel` ×2,
...
Extension models are consumed as a *collection* (`resolveAll(ModelFactory)` → `ModelsProvider` →
...
`registerApiRequestStack` carries *"ORDER IS LOAD-BEARING — extensions must be applied before any
initializer (e.g. ACO) lists + caches the per-request model set."* That exists only because ACO
