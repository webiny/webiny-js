...
class SimpleFile extends React.Component {
    ...
    constructor(props) {
        ...
        ['fileChanged', 'filesChanged', 'getFiles', 'clearFiles'].map(m => this[m] = this[m].bind(this));
    }
    ...
    renderValue() {
        if (this.props.multiple) {
            return this.props.value ? i18n('{files|count:1:file:default:files} selected', { files: _.get(this.props.value, 'length', 0) }) : '';
        }
    ...
    render() {
        ...
        if (this.props.value) {
            clearBtn = (
                <div className={[styles.fileBtn, styles.clearBtn, styles.clearBtnIcon].join(' ')}
...
SimpleFile.defaultProps = {
    ...
    renderError() {
        ...
        return (
            <Alert title={i18n('A file could not be selected')} type="error" close>
                <ul>
    ...
    renderErrors() {
        ...
        return (
            <Alert title={i18n('Some files could not be selected')} type="error" close>
                {data && <ul>{data}</ul>}
            </Alert>
        );
    }
