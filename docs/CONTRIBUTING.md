...
> IMPORTANT: `webiny` should be run from the root of the Webiny project, and since `api` and `apps` folders are a `sandbox` which is present in the project root directory, this is the place to run your `webiny` commands from.
