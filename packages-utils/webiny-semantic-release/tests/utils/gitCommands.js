...
export async function gitCommits(messages) {
    await pReduce(
        messages,
        async (commits, msg) => {
            const stdout = await execa.stdout("git", [
                "commit",
                "-m",
                msg,
                "--allow-empty",
...
export async function gitGetCommits(from) {
    ...
    return (await getStream.array(gitLogParser.parse({ _: `${from ? from + ".." : ""}HEAD` }))).map(
        commit => {
 ...
 * @param {boolean} create `true` to create the branch and switch, `false` to only switch.
 */
