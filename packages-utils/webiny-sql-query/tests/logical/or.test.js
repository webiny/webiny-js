...
describe("$or logical operator test", function() {
    it("should generate correct statement with nested $or operators", () => {
        ...
        assert.equal(
            output,
            `(firstName = 'John' AND lastName = 'Doe' AND (age = 35 OR height = 6.2 OR weight = 225))`
        );
    });

    it("should generate correct statement with root $or operators", () => {
        const output = query.execute({ $or: { firstName: "John", lastName: "Doe" } });
        assert.equal(output, `((firstName = 'John' OR lastName = 'Doe'))`);
    });

    it("should generate correct statement root $or operators", () => {
        const output = query.execute({
            $or: { firstName: "John", lastName: "Doe", $and: { age: 35, height: 6.2, weight: 225 } }
        });
        assert.equal(
            output,
            `((firstName = 'John' OR lastName = 'Doe' OR (age = 35 AND height = 6.2 AND weight = 225)))`
        );
    });
    ...
    it("should generate correct statement with nested $or operators but with values in an array", () => {
        ...
        assert.equal(
            output,
            `(firstName = 'John' AND lastName = 'Doe' AND (age = 35 OR age = 50 OR height = 6.2 OR weight = 225))`
        );
    });
});
