...
import { isDevProxyEnabled } from "../../serve/devProxy/index.js";

/**
 * Standalone hosting-type counterpart to project-aws's `AwsWatch`: where AWS forwards Lambda invocations to
 ...
 * stays hosting-agnostic, like cli-aws) and composed only when the standalone hosting type is registered.
 *
 * ## Where it sits
 *
 ...
 *
 * ```
 *   watchWithHooks  ──▶  StandaloneWatch  ──▶  DefaultWatch
 ...
 *
 * ```
 *   StandaloneWatch                     builds the specs                  (this file)
 ...
 */
export class StandaloneWatch implements Watch.Interface {
    constructor(
    ...
    private devProxySpecs(appName: GetApp.AppName): IServerProcessSpec[] {
        if (!isDevProxyEnabled() || appName !== "api") {
            return [];
        }

        return [{ name: "proxy", spawn: () => spawnDevProxy() }];
    }
}

export const standaloneWatch = Watch.createDecorator({
    decorator: StandaloneWatch,
    dependencies: [GetApp]
});
