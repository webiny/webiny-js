...
/**
 * Standalone hosting-type counterpart to project-aws's `TelemetryNoLongerNewUser`, which hangs off
 * `AdminAfterDeploy`. There is no deploy in the standalone hosting type, so without this the flag
 * would never flip and every standalone user would be reported as a new user forever, on every
 * event, for the lifetime of that machine's `~/.webiny/config`.
 *
 * A successful admin build is the closest standalone equivalent of the AWS milestone: the same app,
