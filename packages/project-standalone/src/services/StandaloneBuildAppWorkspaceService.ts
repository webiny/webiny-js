...
export class StandaloneBuildAppWorkspaceService implements BuildAppWorkspaceService.Interface {
    constructor(
    ...
    async execute(appName: GetApp.AppName, options: BuildAppWorkspaceService.Options = {}) {
        if (appName === "core") {
            throw new Error(
                `The "core" app does not exist in standalone hosting-type Webiny projects. Only "api" and "admin" are available.`
            );
        }
...
export const standaloneBuildAppWorkspaceService = createImplementation({
    abstraction: BuildAppWorkspaceService,
    implementation: StandaloneBuildAppWorkspaceService,
