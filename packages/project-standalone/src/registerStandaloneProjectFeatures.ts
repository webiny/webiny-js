...
import { standaloneBuildAppWorkspaceService } from "./services/StandaloneBuildAppWorkspaceService.js";
import { BuildStandaloneProjectWorkspace } from "./features/BuildStandaloneProjectWorkspace.js";
import { standaloneWatch } from "./features/Watch/StandaloneWatch.js";
import { standaloneServe } from "./serve/StandaloneServe.js";
...
export const registerStandaloneProjectFeatures = (container: Container): void => {
    // Replace the default (AWS) workspace builder with the standalone hosting-type one.
    // The server workspace has no Pulumi scaffolding — only app source templates.
    container.register(standaloneBuildAppWorkspaceService).inSingletonScope();
    ...
    container.registerDecorator(BuildStandaloneProjectWorkspace);
    ...
    // (standalone hosting-type counterpart to project-aws's Lambda invocation forwarding).
    container.registerDecorator(standaloneWatch);

    // Serve built apps as long-running servers (`webiny-standalone serve`). Replace the base
    // DefaultServe (which refuses) with the real standalone hosting-type implementation, then decorate it so
    // the required app builds are asserted before anything is served.
    container.register(standaloneServe).inSingletonScope();
    ...
    // package.json). Deploy-artifact packaging belongs with the standalone hosting type, not the bundler.
