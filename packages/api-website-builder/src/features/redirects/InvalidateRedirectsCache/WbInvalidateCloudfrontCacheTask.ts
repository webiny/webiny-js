...
export interface WbInvalidateCloudfrontCacheInput {
    /**
     * Caller of the task (e.g., `wb.redirects`).
     */
    caller: string;
    /**
...
class WbInvalidateCloudfrontCacheTask implements TaskDefinition.Interface<WbInvalidateCloudfrontCacheInput> {
    id = "wbInvalidateCloudfrontCache";
    title = "Invalidate CloudFront Cache (Website Builder)";
    ...
    public async run({
        input,
        controller
    }: TaskDefinition.RunParams<WbInvalidateCloudfrontCacheInput>) {
...
export const WbInvalidateCloudfrontCacheTaskDefinition = TaskDefinition.createImplementation({
    implementation: WbInvalidateCloudfrontCacheTask,
    dependencies: []
});
