        ...
        try {
            ...
            const result = await context.tasks.listTasks({
                where: {
                    ...
                    definitionId: EntriesTask.MoveEntriesToTrash,
            ...
            return response.done(
                `Task done: entries from "${input.modelId}" model has been moved to the trash bin.`
            );
        } catch (ex) {
            return response.error(
                ex.message ?? "Error while executing MoveEntriesToTrashByModel/ProcessTasks"
            );
        }
    }
}
