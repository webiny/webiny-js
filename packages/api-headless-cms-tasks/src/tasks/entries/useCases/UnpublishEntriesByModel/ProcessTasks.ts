...
import { EntriesTask, IUnpublishEntriesByModelTaskParams } from "~/types";
        ...
        try {
            ...
            const result = await context.tasks.listTasks({
                where: {
                    ...
                    definitionId: EntriesTask.UnpublishEntries,
            ...
            return response.done(
                `Task done: all entries has been unpublished for ${input.modelId} model.`
            );
        } catch (ex) {
            return response.error(
                ex.message ?? "Error while executing UnpublishEntriesByModel/ProcessTasks"
            );
        }
    }
}
