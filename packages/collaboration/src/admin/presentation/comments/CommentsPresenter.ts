...
import { CollaborationApi } from "~/admin/features/api/abstractions.js";
import { CONTENT_TYPE_CMS_ENTRY } from "~/admin/constants.js";
import type { CollabThread, CollabUser } from "~/admin/types.js";
...
class CommentsPresenterImpl implements PresenterAbstraction.Interface {
    ...
    private loadedContentId: string | null = null;
    ...
    async init(contentId: string) {
        if (this.loadedContentId === contentId) {
            return;
        }
        runInAction(() => {
