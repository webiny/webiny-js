...
import { FbFormStepCondition, FbFormStepRule } from "~/renderers/form/types";
...
const checkCondition = (condition: FbFormStepCondition, fieldValue: string) => {
...
const checkIfConditionsMet = ({ formData, rule }: Props) => {
    if (rule.chain === "matchAny") {
        let isValid = true;
