...
import { GetDefaultLanguageRepository as RepositoryAbstraction } from "./abstractions.js";
import { LANGUAGE_MODEL_ID } from "~/shared/constants.js";
import { DefaultLanguageNotFoundError, LanguagePersistenceError } from "~/api/domain/errors.js";
import type { Language } from "~/api/domain/Language.js";

class GetDefaultLanguageRepositoryImpl implements RepositoryAbstraction.Interface {
    constructor(
    ...
    async execute(): RepositoryAbstraction.Return {
        ...
        if (entries.length === 0) {
            return Result.fail(new DefaultLanguageNotFoundError());
        }
...
export const GetDefaultLanguageRepository = RepositoryAbstraction.createImplementation({
    implementation: GetDefaultLanguageRepositoryImpl,
