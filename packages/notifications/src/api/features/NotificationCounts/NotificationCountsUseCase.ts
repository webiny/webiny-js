...
import {
    ...
    NotificationCountsUseCase as UseCase
} from "./abstractions.js";

class NotificationCountsUseCaseImpl implements UseCase.Interface {
    constructor(
        private repository: CountNotificationsRepository.Interface,
    ...
    async execute() {
        ...
        const [inbox, archive, unread] = await Promise.all([
            this.repository.execute({ recipientId, archived: false }),
            this.repository.execute({ recipientId, archived: true }),
            this.repository.execute({ recipientId, archived: false, read: false })
        ]);
...
export const NotificationCountsUseCase = UseCase.createImplementation({
    ...
    dependencies: [CountNotificationsRepository, IdentityContext]
});
