...
const createEntries = async (context: HcmsBulkActionsContext, modelId: string, total = 100) => {
...
const listLatestEntries = async (context: HcmsBulkActionsContext, modelId: string) => {
    const model = await context.cms.getModel(modelId);
...
const listUnpublishedEntries = async (context: HcmsBulkActionsContext, modelId: string) => {
    const model = await context.cms.getModel(modelId);
