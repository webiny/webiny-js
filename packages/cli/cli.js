...
yargs.command(
    ...
    yargs => {
        yargs.positional("name", {
            describe: "The name to be assigned to your generated template."
        });
    },
    async argv => {
        const regexIsCliModule = /(^@webiny\/cli-scaffold-.+$)|(^webiny-cli-scaffold.*$)/;
        const moduleToChoice = module => `${module.scaffold.name} - ${module.scaffold.description}`;

        const webinyModulesNames = (await fs.readdir("../node_modules/@webiny")).map(
            moduleName => `@webiny/${moduleName}`
        );
        const nodeModulesNames = await fs.readdir("../node_modules");
        ...
        const scaffoldModules = scaffoldModulesNames.reduce(
            (modules, crtModuleName) => modules.concat(require(crtModuleName)),
            []
        );
        const choices = scaffoldModules.map(module => moduleToChoice(module));
        if (choices.length === 0)
            throw new Error(
