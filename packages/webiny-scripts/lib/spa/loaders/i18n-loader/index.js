...
function getNamespaces(source) {
    ...
    const output = [];
    _lodash2.default.forEachRight(matches, match => {
        if (!match.name) {
            output.unshift({ from: undefined, name: undefined, to: match.index });
            return true;
        }

        const index = _lodash2.default.findIndex(output, { from: undefined });
        if (index >= 0) {
    ...
    return _lodash2.default.sortBy(output, "from");
}
...
function getNamespaceOnIndex(index, namespaces) {
    let current = null;
    _lodash2.default.forEachRight(namespaces, namespace => {
