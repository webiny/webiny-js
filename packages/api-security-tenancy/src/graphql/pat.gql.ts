...
const plugin: GraphQLSchemaPlugin = {
    type: "graphql-schema",
    name: "graphql-schema-security-pat",
    schema: {
        typeDefs: /* GraphQL */ `
        ...
        resolvers: {
            SecurityUser: {
                personalAccessTokens(user: User, args, context: Context) {
            ...
            SecurityMutation: {
                createPAT: async (root, { data }, context: Context) => {
                ...
                updatePAT: async (root, { id, data }, context: Context) => {
                    const identity = context.security.getIdentity();
                    if (!identity) {
                        throw new Error("Not authorized!");
                    }
                ...
                deletePAT: async (root, { id }, context: Context) => {
                    const identity = context.security.getIdentity();
                    if (!identity) {
                        throw new Error("Not authorized!");
                    }
