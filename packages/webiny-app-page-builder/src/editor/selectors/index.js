...
import type { PbElementType, State } from "webiny-app-page-builder/types";
...
export const getContent = (state: State): Object => {
    ...
    invariant(document, `"pb-page-element-document" plugin must exist for Page Builder to work!`);
 ...
 */
export const getElementWithChildren = (state: State, id: string): PbElementType | null => {
 ...
 */
export const getElement = (state: Object, id: string): PbElementType => {
 ...
 * @returns {*}
 */
export const getParentElementWithChildren = (state: State, id: string): PbElementType => {
    const element = getElement(state, id);
    const content = getContent(state);
