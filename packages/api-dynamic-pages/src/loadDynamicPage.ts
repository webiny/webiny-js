...
export const loadDynamicPage = async (args, context: PbContext) => {
    ...
    const [pages] = await context.pageBuilder.pages.listPublishedPages<DynamicPage>({
        where: { dynamic: true },
        limit: 100
    });
    ...
    try {
        for (let i = 0; i < pages.length; i++) {
            ...
            if (match) {
                const fullPage = await context.pageBuilder.pages.getPublishedPageById<DynamicPage>({
                    id: page.id
                });
                // Load data sources
