...
const createThemeCssV4 = (normalizedFigmaExport, normalizedPrimitivesFigmaExport) => {
    ...
    let themeCss = fs.readFileSync(new URL("./templates/theme-v4.css.txt", import.meta.url), "utf8");
    ...
    // 4. THEME: Colors - Map all color types to --color-* for Tailwind utilities
    {
        ...
        ["backgroundColor", "textColor", "borderColor", "fill", "ringColor"].forEach(type => {
            normalizedFigmaExport
                ...
                .forEach(variable => {
                    ...
                    const colorKey = variable.variantName;
                    if (!colorMap.has(colorKey)) {
                        colorMap.set(
                            colorKey,
                            `--color-${colorKey}: hsl(var(--${varPrefix}-${colorKey}));`
                        );
                    }
                });
        });
