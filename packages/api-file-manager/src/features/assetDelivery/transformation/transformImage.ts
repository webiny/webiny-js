...
/**
 * The visible region (normalized 0–1) for the given framing. Mirrors the geometry
 * in `@webiny/website-builder-sdk`: the largest rectangle of `aspectRatio` that fits
 * inside the crop, centered on the focal point; the crop rectangle itself when no
 * aspect ratio is requested. Kept dependency-free here so the image Lambda never
 * pulls in the SDK.
 */
...
/** True when a framing actually changes the image (a real crop or an aspect ratio). */
...
/**
 * Extract the framed region (crop + optional aspect-ratio-to-focal-point) from an
 * image buffer. Returns the original buffer when the framing is a no-op or the
