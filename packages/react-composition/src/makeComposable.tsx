...
const createEmptyRenderer = (name: string): React.VFC => {
    return {
        [name]: function () {
            useEffect(() => {
...
export function makeComposable<TProps>(
    name: string,
    Component?: React.VFC<TProps & { children?: React.ReactNode }>
) {
    if (!Component) {
    ...
    const Composable: ComposableFC<TProps> = props => {
        ...
        const ComposedComponent = useComponent(Component as React.VFC<TProps>);
