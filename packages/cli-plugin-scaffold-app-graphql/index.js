...
const findEntities = location => {
    ...
    const files = fastGlob.sync(target, {
        unique: true
    });
        ...
        scaffold: {
                ...
                return [
                    {
                        name: "apiLocation",
                        message:
                            "Enter existing GraphQL API location (relative to the project root) or empty for none",
                    ...
                    },
                    {
                        ...
                        validate: (name, { apiLocation }) => {
                            ...
                            try {
                                const names = findEntities(apiLocation).map(
            ...
            generate: async ({ input, oraSpinner }) => {
                ...
                oraSpinner.info(
                    `Open your ${green(
                        appPath
                    )} and pass your new plugins to the app template via the "plugins" option.`
                );
                oraSpinner.info(
