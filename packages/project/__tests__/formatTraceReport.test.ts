...
describe("formatTraceReport", () => {
    ...
    // The filter rounds the same way the row does, so what survives is exactly what would have
    // printed a non-zero figure. Comparing the raw duration against 1 would drop this one.
    it("keeps a phase that rounds up to a millisecond", () => {
        const entries = [entry("hydrate project config", 100, 100.6)];

        const report = formatTraceReport("Webiny CLI", entries, 1000);

        expect(report).toContain("hydrate project config");
        expect(report).toContain("1 ms");
    });
