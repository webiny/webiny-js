...
export class ApplyWcpEnvVars {
    di: IApplyWcpEnvVarsDi;

    constructor(di: IApplyWcpEnvVarsDi) {
        this.di = di;
    }
    ...
    async execute() {
        ...
        const { getProjectIdService, wcpService, loggerService, projectSdkParamsService } =
            this.di;
        ...
        if (license) {
            process.env.WCP_PROJECT_LICENSE = JSON.stringify(license);
        }
