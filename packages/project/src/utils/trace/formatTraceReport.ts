 ...
 *
 * The test is deliberately `Math.round`, matching how `formatRow` prints the duration, so that the
 * rows kept are exactly the rows that would show a non-zero figure. Comparing the raw duration
 * against 1 instead would hide a 0.6 ms phase that the report would have rendered as "1 ms".
 */
