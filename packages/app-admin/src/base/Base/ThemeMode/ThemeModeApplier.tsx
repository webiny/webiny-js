...
import { useBrandVariables } from "./useBrandVariables.js";

/**
 * The single owner of `<html>`'s inline CSS variables. Composes the selected theme's variables
 * with the brand colors registered via `<AdminConfig.Theme.Color />` and applies the result
 * during render (before paint), so it isn't subject to effect-timing races.
 *
...
export const ThemeModeApplier = () => {
    ...
    // Brand colors go on last: they are explicit project configuration and outrank a theme's
    // default accent.
    applyTheme({ ...(registeredVariables ?? cached ?? {}), ...brandVariables });
