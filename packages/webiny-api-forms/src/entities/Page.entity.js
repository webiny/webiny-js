...
import formSettingsFactory from "./FormSettings.model";
...
export const formFactory = (context: Object): Class<IForm> => {
    return class Form extends Entity {
        ...
        constructor() {
            ...
            const { user, forms, security } = context;
            const { Category } = forms.entities;
            ...
            this.attr("isHomeForm")
                .boolean()
                .setDynamic(async () => {
                    const settings = await context.forms.entities.FormsSettings.load();
                    return settings.data.forms.home === this.parent;
                });

            this.attr("isErrorForm")
                .boolean()
                .setDynamic(async () => {
                    const settings = await context.forms.entities.FormsSettings.load();
                    return settings.data.forms.error === this.parent;
                });

            this.attr("isNotFoundForm")
                .boolean()
                .setDynamic(async () => {
                    const settings = await context.forms.entities.FormsSettings.load();
                    return settings.data.forms.notFound === this.parent;
                });
