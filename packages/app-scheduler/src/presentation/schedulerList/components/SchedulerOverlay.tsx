...
export const SchedulerOverlay = observer(({ presenter, title, onExited }: SchedulerOverlayProps) => {
    ...
    return (
        ...
        >
            <Scrollbar onScrollFrame={scrollFrame => onTableScroll({ scrollFrame })}>
                {vm.empty ? <Empty presenter={presenter} /> : <SchedulerTable presenter={presenter} />}
            </Scrollbar>
