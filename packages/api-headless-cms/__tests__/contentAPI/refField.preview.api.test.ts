...
describe(`refField "latest" and "published" entry in "preview" and "read" API`, () => {
    ...
    const updateProduct = async (category: CmsContentEntry, revision: string) => {
        const { updateProduct, createProductFrom } = useProductManageHandler({
            ...manageOpts
        });

        const [createProductFromResponse] = await createProductFrom({ revision });
        const newRevision = createProductFromResponse.data.createProductFrom.data.id;

        const [updateProductResponse] = await updateProduct({
            revision: newRevision,
            data: {
                title: "Potato - updated",
                price: 200,
                availableOn: "2020-12-25",
                color: "white",
                availableSizes: ["s", "m"],
        ...
        return updateProductResponse.data.updateProduct.data;
    };
    ...
    test(`should get review connected to a product in both "read" and "preview" API`, async () => {
        ...
        // Let's create a new revision of product and don't publish it
        const updatedProduct = await updateProduct(category, product.id);

        // "GetReview" on "read" API should still return the original value of product i.e published entry
        const [readGetReviewResponse] = await readGetReview({
            where: {
                id: review.id
            }
        });

        expect(readGetReviewResponse).toEqual({
            data: {
                getReview: {
                    data: {
                        id: review.id,
        ...
        // "GetReview" on "preview" API should return the updated value of product i.e latest entry
        const { getReview: previewGetReview } = useReviewReadHandler({
            ...previewOpts
        });

        const [previewGetReviewResponse] = await previewGetReview({
            where: {
                id: review.id
            }
        });

        expect(previewGetReviewResponse).toEqual({
            data: {
                getReview: {
                    data: {
                        id: review.id,
