...
import { getAssetCropParam, getAssetUrl } from "./deliveryUrl.js";
...
describe("getAssetUrl", () => {
    it("bakes the crop + width + format into the URL", () => {
        const url = getAssetUrl(asset({ top: 0.1, left: 0.2, bottom: 0.1, right: 0.05 }), {
            width: 1200,
            format: "auto"
        });
    ...
    it("omits the crop when there is none", () => {
        expect(getAssetUrl(asset(), { width: 640 })).toBe("https://cdn/x/pic.jpg?width=640");
    });

    it("can opt out of the crop", () => {
        expect(
            getAssetUrl(asset({ top: 0.1, left: 0, bottom: 0, right: 0 }), { crop: false })
        ).toBe("https://cdn/x/pic.jpg");
    });

    it("returns the bare src when no options apply", () => {
        expect(getAssetUrl(asset())).toBe("https://cdn/x/pic.jpg");
    });
    ...
    it("emits aspectRatio + focal (from the asset) when framing to a ratio", () => {
        ...
        expect(getAssetUrl(a, { aspectRatio: "16:9", width: 1200 })).toBe(
    ...
    it("omits focal when it is centered or opted out", () => {
        ...
        expect(getAssetUrl(centered, { aspectRatio: 1 })).toBe(
            "https://cdn/x/pic.jpg?aspectRatio=1"
        );
        const offCenter = { ...asset(), image: { focalPoint: { x: 0.7, y: 0.4 } } };
        expect(getAssetUrl(offCenter, { aspectRatio: 1, focal: false })).toBe(
            "https://cdn/x/pic.jpg?aspectRatio=1"
        );
    });
});
