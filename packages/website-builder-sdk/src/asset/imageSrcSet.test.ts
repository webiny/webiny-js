...
describe("getWebinyImageDimensions", () => {
    it("returns intrinsic dimensions when there is no crop", () => {
        expect(getWebinyImageDimensions(asset())).toEqual({ width: 800, height: 600 });
    });

    it("shrinks by the crop insets", () => {
        expect(
            getWebinyImageDimensions(
                asset({ crop: { top: 0.1, left: 0.25, bottom: 0.1, right: 0.25 } })
            )
    ...
    it("derives height from a target aspect ratio", () => {
        ...
        expect(
            getWebinyImageDimensions(
                asset({ crop: { top: 0, left: 0.25, bottom: 0, right: 0.25 } }),
                {
                    aspectRatio: "16:9"
                }
            )
    ...
    it("returns 0/0 when the intrinsic size is unknown", () => {
        expect(getWebinyImageDimensions({ image: {} })).toEqual({ width: 0, height: 0 });
        expect(getWebinyImageDimensions(null)).toEqual({ width: 0, height: 0 });
    });
});

describe("getWebinyImageSrcSet", () => {
    it("emits the full ladder with crop + format baked into every width", () => {
        const { srcSet, width, height } = getWebinyImageSrcSet(
    ...
    it("trims the ladder to a fixed CSS width (incl. 2x DPR)", () => {
        const { srcSet, src } = getWebinyImageSrcSet(asset(), { cssWidth: "300px" });
    ...
    it("returns empty strings for a missing asset", () => {
        expect(getWebinyImageSrcSet(null)).toEqual({ src: "", srcSet: "", width: 0, height: 0 });
    });
});
