import {
    I18NContext,
    I18NSettings,
    I18NSettingsStorageOperations,
    I18NSettingsStorageOperationsCreateParams,
    I18NSettingsStorageOperationsUpdateParams
} from "@webiny/api-i18n/types";
import { Entity } from "dynamodb-toolbox";
import WebinyError from "@webiny/error";
import defineTable from "~/definitions/table";
import defineSettingsEntity from "~/definitions/settingsEntity";

interface ConstructorParams {
    context: I18NContext;
}

const SORT_KEY = "A";

export class LocalesStorageOperations implements I18NSettingsStorageOperations {
    private readonly _context: I18NContext;
    private _partitionKey: string;
    private readonly _entity: Entity<any>;

    private get partitionKey(): string {
        if (!this._partitionKey) {
            const tenant = this._context.tenancy.getCurrentTenant();
            if (!tenant) {
                throw new WebinyError("Tenant missing.", "TENANT_NOT_FOUND");
            }
            this._partitionKey = `T#${tenant.id}#FM#SETTINGS`;
        }
        return this._partitionKey;
    }

    public constructor({ context }: ConstructorParams) {
        this._context = context;
        const table = defineTable({
            context
        });

        this._entity = defineSettingsEntity({
            context,
            table
        });
    }

    public async get(): Promise<I18NSettings> {
        try {
            const settings = await this._entity.get({
                PK: this.partitionKey,
                SK: SORT_KEY
            });
            if (!settings || !settings.Item) {
                return null;
            }
            return settings.Item;
        } catch (ex) {
            throw new WebinyError(
                ex.message || "Could not fetch the I18N settings.",
                ex.code || "GET_SETTINGS_ERROR"
            );
        }
    }

    public async create({
        data
    }: I18NSettingsStorageOperationsCreateParams): Promise<I18NSettings> {
        const original = await this.get();
        /**
         * TODO: check if need to throw an error on existing settings
         */
        if (original) {
            return await this.update({ original, data });
        }

        try {
            await this._entity.put({
                PK: this.partitionKey,
                SK: SORT_KEY,
                ...data
            });
            return data;
        } catch (ex) {
            throw new WebinyError(
                ex.message || "Cannot create I18N settings.",
                ex.code || "CREATE_FM_SETTINGS_ERROR",
                {
                    data
                }
            );
        }
    }

    public async update({
        data
    }: I18NSettingsStorageOperationsUpdateParams): Promise<I18NSettings> {
        try {
            await this._entity.update({
                PK: this.partitionKey,
                SK: SORT_KEY,
                ...data
            });
            return data;
        } catch (ex) {
            throw new WebinyError(
                ex.message || "Cannot update I18N settings.",
                ex.code || "UPDATE_FM_SETTINGS_ERROR",
                {
                    data
                }
            );
        }
    }

    public async delete(): Promise<void> {
        return this._entity.delete({
            PK: this.partitionKey,
            SK: SORT_KEY
        });
    }
}
