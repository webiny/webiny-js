...
import { useFieldEditor } from "~/modelEditor";
...
export interface FieldEditorDragSource extends DragObjectWithType {
    parent?: string;
...
interface OnFieldDropCallable {
    (source: Partial<FieldEditorDragSource>, target: DropTarget): void;
}
...
interface OnEndDragCallable {
    (item: FieldEditorDragSource, monitor: DragSourceMonitor): void;
}
...
interface IsVisibleCallable {
    (item: FieldEditorDragSource): boolean;
}
interface NoConflictCallable {
    (cb?: IsVisibleCallable): (item: FieldEditorDragSource) => boolean;
}
