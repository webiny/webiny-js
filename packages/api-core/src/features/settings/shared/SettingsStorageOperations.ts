...
import { SettingsStorageOperations as StorageAbstraction } from "./abstractions.js";
import type {
...
export class SettingsStorageOperations implements StorageAbstraction.Interface {
    ...
    async updateSettings({ name, tenant, data }: UpdateSettingsStorageParams): Promise<void> {
        try {
            const keys = {
    ...
    async deleteSettings({ name, tenant }: DeleteSettingsStorageParams): Promise<void> {
        try {
            await deleteItem({
                entity: this.table.getEntity(),
