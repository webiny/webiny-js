...
import { $createBaseParagraphNode, BaseParagraphNode } from "~/nodes/BaseParagraphNode";
    ...
    implements TextNodeThemeStyles, TypographyStylesNode
{
    ...
    static override getType(): string {
        return "base-heading-node";
    }

    static override clone(node: BaseHeadingNode): BaseHeadingNode {
        return new BaseHeadingNode(node.getTag(), node.getTypographyStyleId(), node.__key);
    }
    ...
    static override importJSON(serializedNode: SerializeHeadingNode): BaseHeadingNode {
        const node = $createBaseHeadingNode(serializedNode.tag);
    ...
    ): BaseParagraphNode | HeadingNode {
        // next line for headings are always headings with the same tag
        const newElement = $createBaseHeadingNode(this.getTag());
    ...
    override collapseAtStart(): true {
        const newElement = !this.isEmpty()
            ? $createBaseHeadingNode(this.getTag())
...
export function $createBaseHeadingNode(
    tag: HeadingTagType,
    typographyStyleId?: string
): BaseHeadingNode {
    return $applyNodeReplacement(new BaseHeadingNode(tag, typographyStyleId));
}

export function $isBaseHeadingNode(node: LexicalNode | null | undefined): node is BaseHeadingNode {
    return node instanceof BaseHeadingNode;
}
