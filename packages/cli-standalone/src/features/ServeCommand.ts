...
export class StandaloneServeCommand implements CliCommandFactory.Interface<IServeCommandParams> {
    constructor(
    ...
    async execute(): Promise<CliCommandFactory.CommandDefinition<IServeCommandParams>> {
        return {
            ...
            description: [
                ...
                "",
                "Set PORT to choose which port that URL uses. Run `webiny-standalone build` first."
            ].join("\n"),
...
export const standaloneServeCommand = createImplementation({
    abstraction: CliCommandFactory,
    implementation: StandaloneServeCommand,
