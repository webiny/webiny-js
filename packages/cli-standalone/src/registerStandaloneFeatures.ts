...
import { standaloneWatchCommand } from "./features/WatchCommand.js";
import { standaloneServeCommand } from "./features/ServeCommand.js";

export const registerStandaloneFeatures = (container: Container): void => {
    ...
    container.register(standaloneWatchCommand).inSingletonScope();

    // Serve command: run built apps as long-running servers (production).
    container.register(standaloneServeCommand).inSingletonScope();
};
