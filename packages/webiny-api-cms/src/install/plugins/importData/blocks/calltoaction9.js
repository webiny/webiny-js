export default {
    id: "5c27aa9ba0b03ccaf5c75cc8",
    name: "Call to action #9",
    type: "block",
    content: {
        data: {
            settings: {
                width: { value: "1000px" },
                margin: {
                    advanced: true,
                    desktop: { top: 50, bottom: 50 },
                    mobile: { top: 15, right: 15, bottom: 15, left: 15 }
                }
            }
        },
        elements: [
            {
                data: {
                    settings: {
                        margin: { desktop: { all: 15 }, mobile: { all: 15 } },
                        padding: { desktop: { all: 15 }, mobile: { all: 15 } }
                    }
                },
                elements: [
                    {
                        data: {
                            width: 100,
                            settings: { margin: { desktop: { all: 20 }, mobile: { all: 20 } } }
                        },
                        elements: [
                            {
                                data: {
                                    text: {
                                        object: "value",
                                        document: {
                                            object: "document",
                                            data: {},
                                            nodes: [
                                                {
                                                    object: "block",
                                                    type: "h3",
                                                    data: { align: "center" },
                                                    nodes: [
                                                        {
                                                            object: "text",
                                                            leaves: [
                                                                {
                                                                    object: "leaf",
                                                                    text:
                                                                        "The Glossary Of Telescopes",
                                                                    marks: [
                                                                        {
                                                                            object: "mark",
                                                                            type: "bold",
                                                                            data: {}
                                                                        }
                                                                    ]
                                                                }
                                                            ]
                                                        }
                                                    ]
                                                }
                                            ]
                                        }
                                    },
                                    settings: {
                                        padding: {
                                            desktop: {
                                                all: 0,
                                                top: 0,
                                                right: 0,
                                                bottom: 0,
                                                left: 0
                                            },
                                            mobile: { all: 0, top: 0, right: 0, bottom: 0, left: 0 }
                                        }
                                    }
                                },
                                elements: [],
                                type: "cms-element-text"
                            },
                            {
                                data: {
                                    text: {
                                        object: "value",
                                        document: {
                                            object: "document",
                                            data: {},
                                            nodes: [
                                                {
                                                    object: "block",
                                                    type: "h5",
                                                    data: { align: "center" },
                                                    nodes: [
                                                        {
                                                            object: "text",
                                                            leaves: [
                                                                {
                                                                    object: "leaf",
                                                                    text:
                                                                        "HubbleSite - Out of the ordinary...out of this world.",
                                                                    marks: [
                                                                        {
                                                                            object: "mark",
                                                                            type: "bold",
                                                                            data: {}
                                                                        }
                                                                    ]
                                                                }
                                                            ]
                                                        }
                                                    ]
                                                }
                                            ]
                                        }
                                    },
                                    settings: {
                                        padding: {
                                            desktop: {
                                                all: 0,
                                                top: 0,
                                                right: 0,
                                                bottom: 0,
                                                left: 0
                                            },
                                            mobile: { all: 0, top: 0, right: 0, bottom: 0, left: 0 }
                                        },
                                        margin: {
                                            advanced: true,
                                            desktop: { top: 25, bottom: 25 },
                                            mobile: { top: 15, bottom: 15 }
                                        }
                                    }
                                },
                                elements: [],
                                type: "cms-element-text"
                            }
                        ],
                        type: "cms-element-column"
                    }
                ],
                type: "cms-element-row"
            },
            {
                data: {
                    settings: {
                        margin: {
                            all: 15,
                            advanced: true,
                            desktop: { top: 25 },
                            mobile: { top: 15 }
                        },
                        padding: { all: 15 }
                    }
                },
                elements: [
                    {
                        data: {
                            width: 50,
                            settings: {
                                animation: { name: "fade-right" },
                                margin: { desktop: { all: 20 }, mobile: { all: 20 } },
                                horizontalAlign: "right"
                            }
                        },
                        elements: [
                            {
                                data: {
                                    image: {
                                        name: "Wireframe_136yjq9o84uo.svg",
                                        type: "image/svg+xml",
                                        size: 43073,
                                        src:
                                            "http://localhost:9000/files/Wireframe_136yjq9o84uo.svg",
                                        width: "300",
                                        title: "",
                                        alt: "",
                                        height: "",
                                        align: "center"
                                    },
                                    settings: {
                                        margin: {
                                            desktop: { all: 0 },
                                            mobile: { top: 0, left: 0, right: 0, bottom: 15 },
                                            advanced: true
                                        },
                                        padding: { desktop: { all: 0 }, mobile: { all: 0 } }
                                    }
                                },
                                elements: [],
                                type: "cms-element-image"
                            }
                        ],
                        type: "cms-element-column"
                    },
                    {
                        data: {
                            width: 50,
                            settings: {
                                animation: { name: "fade-left" },
                                margin: { advanced: true, desktop: { right: 0, left: 50 } },
                                verticalAlign: "start",
                                horizontalAlign: "justify"
                            }
                        },
                        elements: [
                            {
                                data: {
                                    text: {
                                        object: "value",
                                        document: {
                                            object: "document",
                                            data: {},
                                            nodes: [
                                                {
                                                    object: "block",
                                                    type: "h4",
                                                    data: {},
                                                    nodes: [
                                                        {
                                                            object: "text",
                                                            leaves: [
                                                                {
                                                                    object: "leaf",
                                                                    text: "Call to Action",
                                                                    marks: []
                                                                }
                                                            ]
                                                        }
                                                    ]
                                                }
                                            ]
                                        }
                                    },
                                    settings: {
                                        padding: { desktop: { all: 0 }, mobile: { all: 0 } },
                                        margin: { desktop: { all: 0 }, mobile: { all: 0 } }
                                    }
                                },
                                elements: [],
                                type: "cms-element-text"
                            },
                            {
                                data: {
                                    text: {
                                        object: "value",
                                        document: {
                                            object: "document",
                                            data: {},
                                            nodes: [
                                                {
                                                    object: "block",
                                                    type: "description",
                                                    data: {},
                                                    nodes: [
                                                        {
                                                            object: "text",
                                                            leaves: [
                                                                {
                                                                    object: "leaf",
                                                                    text:
                                                                        "The Hubble Space Telescope (HST) is a space telescope that was launched into low Earth orbit in 1990 and remains in operation. Although not the first space telescope, Hubble is one of the largest and most versatile and is well known as both a vital research tool and a public relations boon for astronomy. ",
                                                                    marks: []
                                                                }
                                                            ]
                                                        }
                                                    ]
                                                }
                                            ]
                                        }
                                    },
                                    settings: {
                                        padding: {
                                            desktop: {
                                                all: 0,
                                                top: 0,
                                                right: 0,
                                                bottom: 0,
                                                left: 0
                                            },
                                            mobile: { all: 20 }
                                        },
                                        margin: {
                                            advanced: true,
                                            desktop: { top: 25, bottom: 25 },
                                            mobile: { top: 15, bottom: 15, right: 15, left: 15 }
                                        }
                                    }
                                },
                                elements: [],
                                type: "cms-element-text"
                            },
                            {
                                data: {
                                    text: {
                                        object: "value",
                                        document: {
                                            object: "document",
                                            nodes: [
                                                {
                                                    object: "block",
                                                    type: "button",
                                                    nodes: [
                                                        {
                                                            object: "text",
                                                            leaves: [
                                                                { object: "leaf", text: "Click me" }
                                                            ]
                                                        }
                                                    ]
                                                }
                                            ]
                                        }
                                    },
                                    type: "primary",
                                    icon: {
                                        id: ["fas", "arrow-alt-circle-right"],
                                        svg:
                                            '<svg width="15" viewBox="0 0 512 512"><path d="M256 8c137 0 248 111 248 248S393 504 256 504 8 393 8 256 119 8 256 8zM140 300h116v70.9c0 10.7 13 16.1 20.5 8.5l114.3-114.9c4.7-4.7 4.7-12.2 0-16.9l-114.3-115c-7.6-7.6-20.5-2.2-20.5 8.5V212H140c-6.6 0-12 5.4-12 12v64c0 6.6 5.4 12 12 12z" fill="currentColor"></path></svg>',
                                        width: "15",
                                        position: "right"
                                    },
                                    settings: {
                                        horizontalAlignFlex: "flex-start",
                                        padding: { desktop: { all: 0 } },
                                        margin: { desktop: { all: 0 } }
                                    }
                                },
                                elements: [],
                                type: "cms-element-button"
                            }
                        ],
                        type: "cms-element-column"
                    }
                ],
                type: "cms-element-row"
            }
        ],
        type: "cms-element-block",
        source: "5c27aa9ba0b03ccaf5c75cc8"
    },
    preview: {
        name: "cms-element-5c27aa9ba0b03ccaf5c75cc8_6eijqgy10e1.png",
        size: 54298,
        src: "http://localhost:9000/files/cms-element-5c27aa9ba0b03ccaf5c75cc8_6eijqgy10e1.png",
        type: "image/png",
        width: 1000,
        height: 557
    },
    category: "cms-block-category-cta"
};
