export default {
    id: "5c2918aea0b03c430ebc1e22",
    name: "Content #1",
    type: "block",
    content: {
        data: {
            settings: {
                width: { value: "1000px" },
                animation: { name: "fade-up" },
                margin: { mobile: { all: 0 } },
                padding: {
                    advanced: true,
                    desktop: { top: 25, bottom: 25 },
                    mobile: { top: 15, right: 15, bottom: 15, left: 15 }
                }
            }
        },
        elements: [
            {
                data: {
                    settings: {
                        margin: { desktop: { all: 0 }, mobile: { all: 0 } },
                        padding: { desktop: { all: 0 }, mobile: { all: 0 } }
                    }
                },
                elements: [
                    {
                        data: {
                            width: 50,
                            settings: {
                                margin: { desktop: { all: 0 }, mobile: { all: 0 } },
                                padding: { desktop: { all: 0 }, mobile: { all: 0 } }
                            }
                        },
                        elements: [
                            {
                                data: {
                                    settings: {
                                        margin: { desktop: { all: 0 }, mobile: { all: 0 } },
                                        padding: { desktop: { all: 0 }, mobile: { all: 0 } }
                                    }
                                },
                                elements: [
                                    {
                                        data: {
                                            width: 50,
                                            settings: {
                                                margin: {
                                                    advanced: true,
                                                    desktop: { right: 10, bottom: 10 },
                                                    mobile: { bottom: 15 }
                                                },
                                                padding: {
                                                    desktop: {
                                                        all: 10,
                                                        top: 10,
                                                        right: 10,
                                                        bottom: 10,
                                                        left: 10
                                                    },
                                                    mobile: {
                                                        all: 10,
                                                        top: 10,
                                                        right: 10,
                                                        bottom: 10,
                                                        left: 10
                                                    }
                                                },
                                                border: {
                                                    color: "var(--webiny-cms-theme-primary)",
                                                    width: 2,
                                                    style: "solid",
                                                    borders: {
                                                        bottom: false,
                                                        right: false,
                                                        top: false
                                                    }
                                                }
                                            }
                                        },
                                        elements: [
                                            {
                                                data: {
                                                    text: {
                                                        object: "value",
                                                        document: {
                                                            object: "document",
                                                            data: {},
                                                            nodes: [
                                                                {
                                                                    object: "block",
                                                                    type: "h6",
                                                                    data: {},
                                                                    nodes: [
                                                                        {
                                                                            object: "text",
                                                                            leaves: [
                                                                                {
                                                                                    object: "leaf",
                                                                                    text:
                                                                                        "How Does An Lcd Screen Work",
                                                                                    marks: [
                                                                                        {
                                                                                            object:
                                                                                                "mark",
                                                                                            type:
                                                                                                "bold",
                                                                                            data: {}
                                                                                        }
                                                                                    ]
                                                                                }
                                                                            ]
                                                                        }
                                                                    ]
                                                                }
                                                            ]
                                                        }
                                                    },
                                                    settings: {
                                                        padding: {
                                                            desktop: { all: 0 },
                                                            mobile: { all: 0 }
                                                        },
                                                        margin: {
                                                            advanced: true,
                                                            desktop: { bottom: 20 },
                                                            mobile: { bottom: 15 }
                                                        }
                                                    }
                                                },
                                                elements: [],
                                                type: "cms-element-text"
                                            },
                                            {
                                                data: {
                                                    text: {
                                                        object: "value",
                                                        document: {
                                                            object: "document",
                                                            data: {},
                                                            nodes: [
                                                                {
                                                                    object: "block",
                                                                    type: "description",
                                                                    data: {},
                                                                    nodes: [
                                                                        {
                                                                            object: "text",
                                                                            leaves: [
                                                                                {
                                                                                    object: "leaf",
                                                                                    text:
                                                                                        "Did you know that you should try not to sneeze too strongly. Why? A very powerful sneeze has the ability to cause a fracture in your ribcage. ",
                                                                                    marks: []
                                                                                }
                                                                            ]
                                                                        }
                                                                    ]
                                                                }
                                                            ]
                                                        }
                                                    },
                                                    settings: {
                                                        margin: {
                                                            desktop: { all: 0 },
                                                            mobile: { all: 0 }
                                                        },
                                                        padding: {
                                                            desktop: { all: 0 },
                                                            mobile: { all: 0 }
                                                        }
                                                    }
                                                },
                                                elements: [],
                                                type: "cms-element-text"
                                            }
                                        ],
                                        type: "cms-element-column"
                                    },
                                    {
                                        data: {
                                            width: 50,
                                            settings: {
                                                margin: {
                                                    advanced: true,
                                                    desktop: { left: 10, bottom: 10 },
                                                    mobile: { bottom: 15 }
                                                },
                                                padding: {
                                                    desktop: {
                                                        all: 10,
                                                        top: 10,
                                                        right: 10,
                                                        bottom: 10,
                                                        left: 10
                                                    },
                                                    mobile: {
                                                        all: 10,
                                                        top: 10,
                                                        right: 10,
                                                        bottom: 10,
                                                        left: 10
                                                    }
                                                },
                                                border: {
                                                    color: "var(--webiny-cms-theme-primary)",
                                                    width: 2,
                                                    style: "solid",
                                                    borders: {
                                                        bottom: false,
                                                        right: false,
                                                        top: false
                                                    }
                                                },
                                                horizontalAlign: "left"
                                            }
                                        },
                                        elements: [
                                            {
                                                data: {
                                                    text: {
                                                        object: "value",
                                                        document: {
                                                            object: "document",
                                                            data: {},
                                                            nodes: [
                                                                {
                                                                    object: "block",
                                                                    type: "h6",
                                                                    data: {},
                                                                    nodes: [
                                                                        {
                                                                            object: "text",
                                                                            leaves: [
                                                                                {
                                                                                    object: "leaf",
                                                                                    text:
                                                                                        "Help Finding Information Online",
                                                                                    marks: [
                                                                                        {
                                                                                            object:
                                                                                                "mark",
                                                                                            type:
                                                                                                "bold",
                                                                                            data: {}
                                                                                        }
                                                                                    ]
                                                                                }
                                                                            ]
                                                                        }
                                                                    ]
                                                                }
                                                            ]
                                                        }
                                                    },
                                                    settings: {
                                                        padding: {
                                                            desktop: { all: 0 },
                                                            mobile: { all: 0 }
                                                        },
                                                        margin: {
                                                            advanced: true,
                                                            mobile: { bottom: 15 },
                                                            desktop: { bottom: 20 }
                                                        }
                                                    }
                                                },
                                                elements: [],
                                                type: "cms-element-text"
                                            },
                                            {
                                                data: {
                                                    text: {
                                                        object: "value",
                                                        document: {
                                                            object: "document",
                                                            data: {},
                                                            nodes: [
                                                                {
                                                                    object: "block",
                                                                    type: "description",
                                                                    data: {},
                                                                    nodes: [
                                                                        {
                                                                            object: "text",
                                                                            leaves: [
                                                                                {
                                                                                    object: "leaf",
                                                                                    text:
                                                                                        "Over 92% of computers are infected with Adware and spyware. Such software is rarely accompanied by uninstall utility and even when.",
                                                                                    marks: []
                                                                                }
                                                                            ]
                                                                        }
                                                                    ]
                                                                }
                                                            ]
                                                        }
                                                    },
                                                    settings: {
                                                        padding: { desktop: { all: 0 } },
                                                        margin: { desktop: { all: 0 } }
                                                    }
                                                },
                                                elements: [],
                                                type: "cms-element-text"
                                            }
                                        ],
                                        type: "cms-element-column"
                                    }
                                ],
                                type: "cms-element-row"
                            },
                            {
                                data: {
                                    settings: {
                                        margin: { desktop: { all: 0 }, mobile: { all: 0 } },
                                        padding: { desktop: { all: 0 }, mobile: { all: 0 } }
                                    }
                                },
                                elements: [
                                    {
                                        data: {
                                            width: 50,
                                            settings: {
                                                margin: {
                                                    advanced: true,
                                                    desktop: { right: 10, top: 10 },
                                                    mobile: { bottom: 15 }
                                                },
                                                border: {
                                                    color: "var(--webiny-cms-theme-primary)",
                                                    width: 2,
                                                    style: "solid",
                                                    borders: {
                                                        bottom: false,
                                                        right: false,
                                                        top: false
                                                    }
                                                },
                                                padding: {
                                                    desktop: {
                                                        all: 10,
                                                        top: 10,
                                                        right: 10,
                                                        bottom: 10,
                                                        left: 10
                                                    },
                                                    mobile: {
                                                        all: 10,
                                                        top: 10,
                                                        right: 10,
                                                        bottom: 10,
                                                        left: 10
                                                    }
                                                }
                                            }
                                        },
                                        elements: [
                                            {
                                                data: {
                                                    text: {
                                                        object: "value",
                                                        document: {
                                                            object: "document",
                                                            data: {},
                                                            nodes: [
                                                                {
                                                                    object: "block",
                                                                    type: "h6",
                                                                    data: {},
                                                                    nodes: [
                                                                        {
                                                                            object: "text",
                                                                            leaves: [
                                                                                {
                                                                                    object: "leaf",
                                                                                    text:
                                                                                        "Download Anything Now A Days",
                                                                                    marks: [
                                                                                        {
                                                                                            object:
                                                                                                "mark",
                                                                                            type:
                                                                                                "bold",
                                                                                            data: {}
                                                                                        }
                                                                                    ]
                                                                                }
                                                                            ]
                                                                        }
                                                                    ]
                                                                }
                                                            ]
                                                        }
                                                    },
                                                    settings: {
                                                        padding: {
                                                            desktop: { all: 0 },
                                                            mobile: { all: 0 }
                                                        },
                                                        margin: {
                                                            advanced: true,
                                                            desktop: { bottom: 20 },
                                                            mobile: { bottom: 15 }
                                                        }
                                                    }
                                                },
                                                elements: [],
                                                type: "cms-element-text"
                                            },
                                            {
                                                data: {
                                                    text: {
                                                        object: "value",
                                                        document: {
                                                            object: "document",
                                                            data: {},
                                                            nodes: [
                                                                {
                                                                    object: "block",
                                                                    type: "description",
                                                                    data: {},
                                                                    nodes: [
                                                                        {
                                                                            object: "text",
                                                                            leaves: [
                                                                                {
                                                                                    object: "leaf",
                                                                                    text:
                                                                                        "Usage of the Internet is becoming more common due to rapid advancement of technology and the power of globalization. ",
                                                                                    marks: []
                                                                                }
                                                                            ]
                                                                        }
                                                                    ]
                                                                }
                                                            ]
                                                        }
                                                    },
                                                    settings: {
                                                        margin: {
                                                            desktop: { all: 0 },
                                                            mobile: { all: 0 }
                                                        },
                                                        padding: {
                                                            desktop: { all: 0 },
                                                            mobile: { all: 0 }
                                                        }
                                                    }
                                                },
                                                elements: [],
                                                type: "cms-element-text"
                                            }
                                        ],
                                        type: "cms-element-column"
                                    },
                                    {
                                        data: {
                                            width: 50,
                                            settings: {
                                                margin: {
                                                    advanced: true,
                                                    desktop: { left: 10, top: 10 },
                                                    mobile: { bottom: 15 }
                                                },
                                                border: {
                                                    color: "var(--webiny-cms-theme-primary)",
                                                    width: 2,
                                                    style: "solid",
                                                    borders: {
                                                        bottom: false,
                                                        right: false,
                                                        top: false
                                                    }
                                                },
                                                padding: {
                                                    desktop: {
                                                        all: 10,
                                                        top: 10,
                                                        right: 10,
                                                        bottom: 10,
                                                        left: 10
                                                    },
                                                    mobile: {
                                                        all: 10,
                                                        top: 10,
                                                        right: 10,
                                                        bottom: 10,
                                                        left: 10
                                                    }
                                                }
                                            }
                                        },
                                        elements: [
                                            {
                                                data: {
                                                    text: {
                                                        object: "value",
                                                        document: {
                                                            object: "document",
                                                            data: {},
                                                            nodes: [
                                                                {
                                                                    object: "block",
                                                                    type: "h6",
                                                                    data: {},
                                                                    nodes: [
                                                                        {
                                                                            object: "text",
                                                                            leaves: [
                                                                                {
                                                                                    object: "leaf",
                                                                                    text:
                                                                                        "How To Put Movies On Iphone",
                                                                                    marks: [
                                                                                        {
                                                                                            object:
                                                                                                "mark",
                                                                                            type:
                                                                                                "bold",
                                                                                            data: {}
                                                                                        }
                                                                                    ]
                                                                                }
                                                                            ]
                                                                        }
                                                                    ]
                                                                }
                                                            ]
                                                        }
                                                    },
                                                    settings: {
                                                        padding: {
                                                            desktop: { all: 0 },
                                                            mobile: { all: 0 }
                                                        },
                                                        margin: {
                                                            advanced: true,
                                                            desktop: { bottom: 20 },
                                                            mobile: { left: 15 }
                                                        }
                                                    }
                                                },
                                                elements: [],
                                                type: "cms-element-text"
                                            },
                                            {
                                                data: {
                                                    text: {
                                                        object: "value",
                                                        document: {
                                                            object: "document",
                                                            data: {},
                                                            nodes: [
                                                                {
                                                                    object: "block",
                                                                    type: "description",
                                                                    data: {},
                                                                    nodes: [
                                                                        {
                                                                            object: "text",
                                                                            leaves: [
                                                                                {
                                                                                    object: "leaf",
                                                                                    text:
                                                                                        "According to the research firm Frost & Sullivan, the estimated size of the North American used test and measurement equipment market",
                                                                                    marks: []
                                                                                }
                                                                            ]
                                                                        }
                                                                    ]
                                                                }
                                                            ]
                                                        }
                                                    },
                                                    settings: {
                                                        padding: {
                                                            desktop: { all: 0 },
                                                            mobile: { all: 0 }
                                                        },
                                                        margin: {
                                                            desktop: { all: 0 },
                                                            mobile: { all: 0 }
                                                        }
                                                    }
                                                },
                                                elements: [],
                                                type: "cms-element-text"
                                            }
                                        ],
                                        type: "cms-element-column"
                                    }
                                ],
                                type: "cms-element-row"
                            }
                        ],
                        type: "cms-element-column"
                    },
                    {
                        data: {
                            width: 49.99999999999993,
                            settings: {
                                margin: {
                                    advanced: true,
                                    desktop: { left: 25 },
                                    mobile: { top: 0, left: 0 }
                                },
                                verticalAlign: "center"
                            }
                        },
                        elements: [
                            {
                                data: {
                                    image: {
                                        name: "image-placeholder_146fjq9q90jt.svg",
                                        type: "image/svg+xml",
                                        size: 3140,
                                        src:
                                            "http://localhost:9000/files/image-placeholder_146fjq9q90jt.svg"
                                    },
                                    settings: {
                                        margin: { desktop: { all: 0 }, mobile: { all: 0 } },
                                        padding: { desktop: { all: 0 }, mobile: { all: 0 } }
                                    }
                                },
                                elements: [],
                                type: "cms-element-image"
                            }
                        ],
                        type: "cms-element-column"
                    }
                ],
                type: "cms-element-row"
            }
        ],
        type: "cms-element-block"
    },
    preview: {
        name: "cms-element-5c2918aea0b03c430ebc1e22_d8tjqb9vcvq.png",
        size: 72954,
        src: "http://localhost:9000/files/cms-element-5c2918aea0b03c430ebc1e22_d8tjqb9vcvq.png",
        type: "image/png",
        width: 1000,
        height: 415
    },
    category: "cms-block-category-content"
};
