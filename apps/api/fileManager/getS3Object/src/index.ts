...
exports.handler = async event => {
    ...
    // object_get_context['inputS3Url'] is the presigned URL for the requested object.
    ...
    try {
        ...
        console.log('ideoeeeeeeee', presignedUrl)
        const s3ObjRsp = await axios.get(presignedUrl, {
            responseType: "arraybuffer"
        });


        console.log(JSON.stringify(s3UrlParsed))
        // POC for Magic Links
        if (s3UrlParsed.pathname.includes("traverse")) {
            // @ts-ignore
            if (s3UrlParsed.query.magic !== '123') {
