...
class SelfCleaningTaskImpl implements TaskDefinition.Interface {
    ...
    public readonly description = "A task which will remove db records related to itself after execution.";
    ...
    public async run(params: TaskDefinition.RunParams): Promise<TaskDefinition.Result> {
        ...
        if (entriesResult.isFail()) {
            return controller.response.error(`Failed to list latest CMS entries for model ${model.modelId}.`);
        }
        ...
        return controller.response.done(`Found entry with a title: ${entry.values[titleFieldId] || entry.id}`);
    }
}
