...
const awsRegionRegion = awsRegions.lookup({ code: "eu-central-1" });
const defaultVpc = new aws.ec2.DefaultVpc("default-vpc");
const defaultSubNets = awsRegionRegion.zones.map(
    availabilityZone =>
        new aws.ec2.DefaultSubnet(`default-${availabilityZone}`, { availabilityZone })
);

const docDbCluster = new aws.docdb.Cluster("documentdb-cluster", {
    backupRetentionPeriod: 1,
    clusterIdentifier: "documentdb-cluster",
    engine: "docdb",
    masterPassword: String(process.env.DOCUMENT_DB_PASSWORD), // Secrets Manager
    masterUsername: String(process.env.DOCUMENT_DB_USERNAME), // Secrets Manager
...
const databaseProxy = new aws.lambda.Function("document-db-proxy", {
    ...
    vpcConfig: {
        subnetIds: defaultSubNets.map(subNet => subNet.id),
        securityGroupIds: [defaultVpc.defaultSecurityGroupId]
    },
    environment: {
        variables: {
            DOCUMENT_DB_SERVER: docDbCluster.endpoint,
