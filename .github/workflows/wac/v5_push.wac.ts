...
export const v5_Push = createWorkflow({
    name: `(v5) Push`,
    on: { push: { branches: ["v5-next", "v5-dev"] } },
    jobs: {
        constants: createJob({
            ...
            steps: [
                {
                    ...
                    id: "global-cache-key",
                    run: `echo "global-cache-key=${WEBINY_V5}-\${{ runner.os }}-$(/bin/date -u "+%m%d")-\${{ vars.RANDOM_CACHE_KEY_SUFFIX }}" >> $GITHUB_OUTPUT`
                },
                {
