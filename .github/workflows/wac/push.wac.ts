...
const createE2EJobs = (storageOps: AbstractStorageOps) => {
  ...
  const constantsJob: NormalJob = createJob({
    ...
    outputs: {
      "cypress-folders":
        "${{ steps.list-cypress-folders.outputs.cypress-folders }}",
      "pulumi-backend-url":
        "${{ steps.pulumi-backend-url.outputs.pulumi-backend-url }}",
    },
    steps: [
      {
  ...
  const projectSetupJob: NormalJob = createJob({
    ...
    outputs: {
      "cypress-config":
        "${{ steps.save-cypress-config.outputs.cypress-config }}",
    },
    environment: "next",
    env,
    checkout: {
...
const createVitestTestsJobs = (storageOps?: AbstractStorageOps) => {
  const jobNames = {
    constants: ["vitest", storageOps?.shortId, "constants"]
      .filter(Boolean)
      .join("-"),
  ...
  if (storageOps) {
    ...
    if (storageOps.id === "ddb-os,ddb") {
      env["AWS_OPENSEARCH_DOMAIN_NAME"] =
        "${{ secrets.OPENSEARCH_DOMAIN_NAME }}";
      env["OPENSEARCH_ENDPOINT"] = "${{ secrets.OPENSEARCH_ENDPOINT }}";
      env["OPENSEARCH_USERNAME"] = "${{ secrets.OPENSEARCH_USERNAME }}";
      env["OPENSEARCH_PASSWORD"] = "${{ secrets.OPENSEARCH_PASSWORD }}";
...
export const push = createWorkflow({
  ...
  jobs: {
    constants: createJob({
      ...
      outputs: {
        "global-cache-key":
          "${{ steps.global-cache-key.outputs.global-cache-key }}",
      ...
      steps: [
        {
          ...
          id: "global-cache-key",
          run: `echo "global-cache-key=\${{ github.ref_name }}-\${{ runner.os }}-$(/bin/date -u "+%m%d")-\${{ vars.RANDOM_CACHE_KEY_SUFFIX }}" >> $GITHUB_OUTPUT`,
        },
        {
