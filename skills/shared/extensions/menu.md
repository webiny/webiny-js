...
A menu entry here is stored configuration, not code. You supply the fields and the admin renders
it; there is no component to write and nothing to import.

- Pick a `name` nobody else is using. It is an identifier, so camelCase, no spaces.
- `to` must be a path that already resolves. You are not creating the page. If the user is asking
  for a destination that does not exist yet, say so rather than producing an item that leads
  nowhere.
- Use `parent` only for one of the groups listed above. A made-up parent hides the item completely.
- Icons are not available here. A generated menu entry is text only.
